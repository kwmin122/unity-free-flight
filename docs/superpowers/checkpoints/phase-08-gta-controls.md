...
- `W/S` changes throttle, `A/D` requests a turn, `Up/Down` controls pitch, `Q/E` gives manual roll, `Space` brakes on the ground, and `R` resets.
...
- Updated HUD state text with throttle percent and a compact control hint including the existing `R` reset.
...
- HUD reset hint targeted test: `/tmp/MINgo-test-run/Builds/TestResults/editmode-hud-reset-hint.xml` passed 1/1.
- Full unfiltered EditMode in temp copy: `/tmp/MINgo-test-run/Builds/TestResults/editmode-gta-controls-temp-unfiltered-4.xml` passed 99/99, including 56/56 MCP Unity package tests.
- macOS temp build: `/tmp/mingo-build-gta-controls-temp-4.log` ended with `Build Finished, Result: Success.`
